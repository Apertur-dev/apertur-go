...
// Share shares an upload session with a guest via email or SMS. opts.Channel
// must be "email" or "sms"; opts.SmsConsent must be true when Channel is
// "sms" (enforced server-side). opts.Note is only used for email shares.
func (s *SessionsResource) Share(ctx context.Context, uuid string, opts ShareSessionOptions) (*ShareResult, error) {
	var result ShareResult
	if err := s.http.request(ctx, "POST", fmt.Sprintf("/api/v1/upload-sessions/%s/share", uuid), opts, &result); err != nil {
		return nil, err
	}
	return &result, nil
}
...
//
// When opts.PollFrom is set to an ISO 8601 timestamp, the server long-polls for
// up to 5 minutes waiting for a change since that cursor. Callers that want to
// long-poll should ensure the supplied context has a timeout of at least 6
// minutes (e.g. context.WithTimeout(ctx, 6*time.Minute)) so the server releases
// the response first under the happy path.
//
...
func (s *SessionsResource) DeliveryStatus(ctx context.Context, uuid string, opts ...DeliveryStatusOptions) (*DeliveryStatusResponse, error) {
	path := fmt.Sprintf("/api/v1/upload-sessions/%s/delivery-status", uuid)
	if len(opts) > 0 && opts[0].PollFrom != "" {
		qs := url.Values{}
		qs.Set("pollFrom", opts[0].PollFrom)
		path += "?" + qs.Encode()
	}

	var result DeliveryStatusResponse
	if err := s.http.request(ctx, "GET", path, nil, &result); err != nil {
		return nil, err
	}
	return &result, nil
