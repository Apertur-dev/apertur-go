...
Set `SigningSecret` to enable HMAC request signing. When configured, every
outgoing JSON or no-body request is automatically signed, adding
`X-Aptr-Signature` and `X-Aptr-Timestamp` headers so the server can verify the
request's authenticity. Leave it unset for backwards-compatible, unsigned
requests.
...
// Share a session with a guest via email or SMS
smsConsent := true
share, err := client.Sessions.Share(ctx, session.UUID, apertur.ShareSessionOptions{
	Channel:    "sms",
